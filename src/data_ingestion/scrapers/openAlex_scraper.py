...
class openAlexScraper(BaseScraper):
    ...
    def fetch_data(self) -> list[dict]:
        ...
        count_result = 0
        result = []
        while count_result <= MAX_RESULTS:
            try:
                works = Works().filter(concepts = {"id":FIELD_ID}).select(DATA_NEED).get(per_page = 50,cursor = cursor)
                if not list(works):
                ...
                for i in range(1,MAX_RETRIES + 1):
                    print(f"Retrying {i}/{MAX_RETRIES} after {i*DELAY} seconds")
                    time.sleep(i*DELAY)
                    try:
                        works = Works().filter(concepts={"id": FIELD_ID}).select(DATA_NEED).get(per_page=50, cursor=cursor)
                        break
